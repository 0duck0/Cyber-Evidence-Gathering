# This script was patched together from multiple code sources over a period of a few years. 
...
$savetoserver = \\\PATH TO SAVE Evidence to\\\
...
$head = '<style> BODY{font-family:caibri; background-color:Aliceblue;} TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;} TH{font-size:1.1em;border-width: 1px;padding: 2px;border-style: solid;border-color: black;background-color:PowderBlue} TD{border-width: 1px;padding: 2px;border-style: 
solid;border-color: black;background- color:white} </style>'

$OutLevel1 = "c:\windows\temp\$CompName-$User-$Date-Level1.html"
...
ConvertTo-Html -Head $head -Title "Live Response script for $CompName.$User" -Body "<h1> Live Forensics Script <p> Computer Name : $CompName &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp User ID : $User </p> </h1>" > $OutLevel1
...
ConvertTo-html -Body "<H2> Current Date and Time</H2>" >> $OutLevel1 
...
gp -ea 0 'hklm:\system\currentcontrolset\control\session manager\bootexecute' | select * -ExcludeProperty PS* | ConvertTo-html -Body "<H2> Startup Applications - hklm-system-currentcontrolset-control-sessionmanager-bootexecute </H2>" >> $OutLevel1
...
gp -ea 0 'hklm:\software\microsoft\windows\currentversion\runonce' | select * -ExcludeProperty PS* | ConvertTo-html -Body "<H2> Startup Applications - hklm-software-microsoft-windows-currentversion-runonce </H2>" >> $OutLevel1
...
gp -ea 0 'hklm:\software\microsoft\windows\currentversion\group policy\startup' | select * -ExcludeProperty PS* | ConvertTo-html -Body "<H2> Startup Applications - Additional for 64 bit Systems </H2>" >> $OutLevel1
...
gp -ea 0 'hkcu:\software\microsoft\windows\currentversion\run' | select * -ExcludeProperty PS* | ConvertTo-html -Body "<H2> Startup Applications - Additional for 64 bit Systems </H2>" >> $OutLevel1
...
'Remote IP : Port#'=$data[2]; 'Process ID'= $data[4];
...
gwmi -ea 0 win32_process | select processname,@{NAME='CreationDate';EXPRESSION={$_.ConvertToDateTime($_.CreationDate)}},ProcessId,ParentProcessId,CommandLine,sessionID |
sort ParentProcessId -desc | ConvertTo-html -Body "<H2> Running Processes sorted by ParentProcessID</H2>" >> $OutLevel1

gwmi -ea 0 win32_process | where {$_.name -eq 'svchost.exe'} | select ProcessId |foreach-object {$P = $_.ProcessID ;gwmi win32_service |where {$_.processId -eq $P} | select processID,name,DisplayName,state,startmode,PathName} | 
ConvertTo-html -Body "<H2> Running SVCHOST and associated Processes </H2>" >> $OutLevel1

gwmi -ea 0 win32_Service | select Name,ProcessId,State,DisplayName,PathName | sort state | 
ConvertTo-html -Body "<H2> Running Services - Sorted by State</H2>" >> $OutLevel1

driverquery.exe /v /FO CSV | ConvertFrom-CSV | Select 'Display Name','Start Mode', Path | sort Path | 
ConvertTo-html -Body "<H2> Drivers running, Startup mode and Path - Sorted by Path </H2>" >> $OutLevel1

gci -r -ea 0 c:\ -include *.dll | select Name,CreationTime,LastAccessTime,Directory | sort CreationTime -desc | select -first 50 |
ConvertTo-html -Body "<H2> Last 50 DLLs created - Sorted by CreationTime </H2>" >> $OutLevel1

openfiles /query > "c:\windows\temp\$CompName-$User-$Date-OpenFiles.txt"

gwmi -ea 0 Win32_Share | select name,path,description | 
ConvertTo-html -Body "<H2> Open Shares</H2>" >> $OutLevel1

gp -ea 0 'hkcu:\Software\Microsoft\Windows\CurrentVersion\explorer\Map Network Drive MRU' | select * -ExcludeProperty PS* | 
ConvertTo-html -Body "<H2> Mapped Drives </H2>" >> $OutLevel1

gwmi -ea 0 Win32_ScheduledJob | 
ConvertTo-html -Body "<H2> Scheduled Jobs </H2>" >> $OutLevel1

get-winevent -ea 0 -logname Microsoft-Windows-TaskScheduler/Operational | select TimeCreated,ID,Message | 
ConvertTo-html -Body "<H2> Scheduled task events   </H2>" >> $OutLevel1

Get-HotFix -ea 0| Select HotfixID, Description, InstalledBy, InstalledOn | Sort-Object InstalledOn -Descending | 
ConvertTo-html -Body "<H2> HotFixes applied - Sorted by Installed Date </H2>" >> $OutLevel1

gp -ea 0    HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*    | Select DisplayName,DisplayVersion,Publisher,InstallDate,InstallLocation | Sort InstallDate -Desc | 
ConvertTo-html -Body "<H2> Installed Applications - Sorted by Installed Date </H2>" >> $OutLevel1

gwmi -ea 0 Win32_ShortcutFile | select FileName,caption,@{NAME='CreationDate';EXPRESSION={$_.ConvertToDateTime($_.CreationDate)}},@{NAME=’LastAccessed’;EXPRESSION={$_.ConvertToDateTime($_.LastAccessed)}},@{NAME=’LastModified’;EXPRESSION={$_.ConvertToDateTime($_.LastModified)}
},Target | Where-Object {$_.lastModified -gt ((Get-Date).addDays(-5)) }| sort LastModified -Descending |
ConvertTo-html -Body "<H2> Link File Analysis -Last 5 days </H2>" >> $OutLevel1

gci -Path C:\ -r -ea 0 -include $ExecutableFiles |Where {$_.Attributes -band [IO.FileAttributes]::Compressed} |
ConvertTo-html -Body "<H2> Compressed files</H2>" >> $OutLevel1

gci -Path C:\ -r -force -ea 0 -include $ExecutableFiles |Where {$_.Attributes -band [IO.FileAttributes]::Encrypted} | 
ConvertTo-html -Body "<H2> Encrypted files </H2>" >> $OutLevel1

gwmi -ea 0 Win32_ShadowCopy | select DeviceObject,@{NAME='CreationDate';EXPRESSION={$_.ConvertToDateTime($_.InstallDate)}} | 
ConvertTo-html -Body "<H2> ShadowCopy List </H2>" >> $OutLevel1

gci -path C:\windows\prefetch\*.pf -ea 0 | select Name, LastAccessTime,CreationTime | sort LastAccessTime | 
ConvertTo-html -Body "<H2>Prefetch Files </H2>" >> $OutLevel1

ipconfig /displaydns | select-string 'Record Name' | Sort |
ConvertTo-html -Body "<H2> DNS Cache</H2>" >> $OutLevel1

Get-WinEvent -max 50 -ea 0 -FilterHashtable @{Logname='system';ID=1014} | select TimeCreated,ID,Message | 
ConvertTo-html -Body "<H2> Event log – DNS – failed resolution events </H2>" >> $OutLevel1
...
$la = $env:LOCALAPPDATA ;gci -r -ea 0 '$la\Microsoft\Windows\Temporary Internet Files' | select Name, LastWriteTime, CreationTime,Directory| Where-Object{$_.lastwritetime -gt ((Get-Date).addDays(-5)) }| Sort creationtime -Desc | 
ConvertTo-html -Body "<H2> Temporary Internet Files - Last 5 days - Sorted by CreationTime </H2>" >> $OutLevel1

$a = $env:APPDATA;
gci -r -ea 0 '$a\Microsoft\Windows\cookies' | select Name |foreach-object {$N = $_.Name ;
get-content -ea 0 '$a\Microsoft\Windows\cookies\$N' | select-string '/'} | 
ConvertTo-html -Body "<H2> Cookies </H2>" >> $OutLevel1

gp -ea 0 'hkcu:\Software\Microsoft\Internet Explorer\TypedUrls' | select * -ExcludeProperty PS* | 
ConvertTo-html -Body "<H2> Typed URLs </H2>" >> $OutLevel1 
write-host ""
Write-host "**** Script is running please wait ****"

gp -ea 0 'hkcu:\Software\Microsoft\Windows\CurrentVersion\Internet Settings' | select * -ExcludeProperty PS* |
ConvertTo-html -Body "<H2> Important Registry keys - Internet Settings </H2>" >> $OutLevel1

gci -ea 0 "hkcu:SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\ZoneMap\EscDomains" | 
select PSChildName | ConvertTo-html -Body "<H2>
Important Registry keys - Internet Trusted Domains </H2>" >> $OutLevel1

gp -ea 0 'hklm:\Software\Microsoft\Windows NT\CurrentVersion\Windows' | select AppInit_DLLs | 
ConvertTo-html -Body "<H2> Important Registry keys -
AppInit_DLLs </H2>" >> $OutLevel1

gp -ea 0 'hklm:\Software\Microsoft\Windows\CurrentVersion\policies\system' | select * -ExcludeProperty PS* | 
ConvertTo-html -Body "<H2> Important Registry keys - UAC Group Policy Settings </H2>" >> $OutLevel1

gp -ea 0 'HKLM:\Software\Microsoft\Active Setup\Installed Components\*' | select ComponentID,'(default)',StubPath | ConvertTo-html -Body "<H2>gci c:\ -r -ea 0 -include $ExecutableFiles |foreach {$P = $_.fullname; get-item $P -Stream *} |where {$_.Stream -match "Zone.Identifier"}
...
Get-WinEvent -max 50 -ea 0 -FilterHashtable @{Logname='security';ID=4616} | select TimeCreated,ID,Message | ConvertTo-html -"<H2> Event log - The system time was changed</H2>" >> $OutLevel1 
...
get-winevent -ea 0 -logname Microsoft-Windows-Application-Experience/Program- Inventory | select TimeCreated,ID,Message | ConvertTo-html -Body "<H2>Application inventory events   </H2>" >> $OutLevel1


get-winevent -ea 0 -logname Microsoft-Windows-TerminalServices- LocalSessionManager | select TimeCreated,ID,Message | ConvertTo-html -Body "<H2> Terminal services events   </H2>" >> $OutLevel1
...
copy-item "c:\windows\temp\$CompName-$User-$Date-Level1.html" "$savetoserver\$CompName-$User-$Date-Level1.html" -force -verbose
copy-item "c:\windows\temp\$CompName-$User-$Date-NetworkConnections.txt" "$savetoserver\$CompName-$User-$Date-NetworkConnections.txt" -force -verbose
copy-item "c:\windows\temp\$CompName-$User-$Date-DNSCache.txt" "$savetoserver\$CompName-$User-$Date-DNSCache.txt" -force -verbose
copy-item "c:\windows\temp\$CompName-$User-$Date-FirewallConfig.txt" "$savetoserver\$CompName-$User-$Date-FirewallConfig.txt" -force -verbose
copy-item "c:\windows\temp\$CompName-$User-$Date-HostsFile.txt" "$savetoserver\$CompName-$User-$Date-HostsFile.txt.txt" -force -verbose
copy-item "c:\windows\temp\$CompName-$User-$Date-AuditPolicy.txt" "$savetoserver\$CompName-$User-$Date-AuditPolicy.txt" -force -verbose
copy-item "c:\windows\temp\$CompName-$User-$Date-NetworkConnections.txt" "$savetoserver\$CompName-$User-$Date-NetworkConnections.txt" -force -verbose
